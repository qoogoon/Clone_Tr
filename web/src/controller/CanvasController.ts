...
export default class CanvasController {
    ...
    isCardFloating: boolean = false
    floatingListIndex: number = -1
    floatingCardIndex: number = -1
    listFloatingOffset: ElementPosition = { left: 0, top: 0 }
    cardFloatingOffset: ElementPosition = { left: 0, top: 0 }
    lists: List[] = []
    arrListData: ListData[] = []
    arrCardData: CardData[] = []
    addCardListIndex: number = -1
    scrollX: number = 0
    constructor() {
        this.loadFromServer()
    }
     ...
     */
    setCardListener(element: HTMLElement, data: CardData, listIndex: number) {
    ...
    private handleListFloatingEnd = async () => {
        ...
        await CardModel.setCards(this.arrCardData).catch((reason) => { handleError(reason) })
        this.loadFromServer()
    }
     ...
     * Floating 카드 종료 핸들러
     */
    private handleCardFloatingEnd = async () => {
     ...
     */
    private handleCardFloating = async (moveEvent: MouseEvent) => {
        const { isCardFloating, cardFloatingOffset } = this
        if (!isCardFloating) return
        if (this.floatingCardIndex === -1) return
        ...
        if (isExchangeTop || isExchangeBottom) {
            ...
            this.moveCard({ ...movingCard }, isExchangeTop ? "top" : "bottom")
        }
    }
     ...
     */
    private moveCard(card: CardData, dir: "top" | "bottom") {
     ...
     */
    private setCardData(editData: CardData) {
     ...
     * @param editData 수정 할 데이터 
     */
    private setList(editData: ListData) {
